...
- Prior experience (CORRECTED 2026-04-07): masking modes (mask_nov_jan / mask_bf_jan) tend to
  ...
  baseline," but the retail engagement's investigation (Issue #51) traced the root cause to a latent zero-injection
...
- ~700 days with mask_bf_jan: FPR ~93% (⚠️ CORRECTED: originally attributed to "masking creates
  gaps" — the retail engagement's Issue #51 investigation showed this is partially a data-prep zero-injection
...
- When ALL methods show a high naive FPR, do **not** abandon the result — shift the *headline*
  to the miscalibration-immune reads: the rolling-backtest **empirical-null RANK**, the
  **specification grid** (sign + magnitude across pre-registered specs), **direction consistency**,
  and **prob_positive**. Effect-size ranking is unaffected by the interval miscalibration that
  drives the FPR [Abadie 2010; Young 2019]. A date-shuffled randomization test is an *optional*
  secondary cross-check, not a gate — on autocorrelated/masked series it often tests the wrong
  question (see "Three tests, three different questions" and "Diagnose, don't demote" below).
...
| Date-shuffled p-value (optional) | ≤ 0.10 | Secondary cross-check; conservative on autocorrelated data — prefer the RANK + spec grid |

> **The empirical-null backtest RANK and the pre-period placebo FPR are both REQUIRED;
> a date-shuffled randomization test is an optional secondary cross-check.**
> - Backtest RANK (real effect vs an empirical null of rolling-window placebos): the
>   miscalibration-immune headline statistic — effect-size ranking is unaffected by systematic
>   interval under-coverage [Abadie 2010; Young 2019]. Lead with this (and the spec grid) when FPR
>   is high.
> - Pre-period placebo FPR: checks whether the model's NAIVE per-window p-values are calibrated.
>   Uses model p-values by design — that's what it's measuring. A high FPR ⇒ distrust a single
>   naive interval, NOT ⇒ the result is unreliable.
> - BSTS naive p-values alone are unreliable — multi-method testing (BSTS VI/HMC, Prophet, RDiT)
>   shows 35-55% naive per-window FPR on daily retail revenue. The fix is to read the RANK + spec
>   grid, not to switch model class.
> - Date-shuffled randomization shares the effect-size-ranking immunity in principle, but on
>   autocorrelated/masked series it can answer the wrong question (it returned p≈0.47 for a
>   genuinely-positive effect whose backtest rank was 0.94 in one engagement). Prefer the
>   rolling-window RANK, which preserves temporal structure. See "Three tests, three different
>   questions" and "Diagnose, don't demote".

Passing all checks = strong result. Passing 4+ with consistent direction = defensible.
Failing the FPR check means the model's NAIVE p-values are inflated — stop quoting per-window
significance and lead with the calibrated reads (backtest RANK, spec grid, prob_positive). It does
NOT by itself invalidate a result whose RANK passes; **diagnose, don't demote** (see the
Methodology Communication section) rather than spec-shopping for a passing spec.
...
### When the interval crosses zero (inconclusive ≠ null)

A credible/confidence interval that crosses zero but sits mostly positive is a **legitimate
finding**, not a failure: "probably positive, not yet conclusive." Report it as such.

- **Do NOT claim "no effect" from a zero-crossing interval.** Absence of significance is not
  evidence of absence [Altman & Bland 1995]. To assert the effect is *negligible* you need an
  **equivalence test (TOST)** against a smallest effect of interest, with adequate power
  [Lakens 2017] — a non-significant interval alone cannot support "it didn't work."
- **Lead with estimation, not the threshold** [Cumming 2014]: point estimate + interval +
  probability-of-direction ("~X% probability the lift > 0"), plus the spec grid's sign-consistency
  and median across specs.

**Cross-checks that strengthen an inconclusive-but-positive read (run alongside BSTS):**
- **ITS / segmented regression** — model a level change (β₂) and a slope change (β₃); the slope
  term directly captures a gradual adoption ramp [Lopez Bernal 2017; Wagner 2002]. Agreement with
  BSTS on sign + rough magnitude = convergent evidence. Caveat: ITS **extrapolates the pre-trend
  forward** (its own linearity assumption), so it is a cross-check, not a strictly-better
  replacement.
- **T₀ sensitivity** — re-run varying the *real* launch date ±2 weeks and report **all** dates;
  if the conclusion flips, the data can't answer it; if it holds, that's real robustness. This is
  **distinct** from the fake-T₀ pre-period placebo (a calibration check) [Abadie 2021].
- **Granularity discipline** — do not switch daily→weekly *after* seeing a daily result cross
  zero; that's fishing (a garden-of-forking-paths move — see "Diagnose, don't demote"). Weekly
  aggregation legitimately reduces autocorrelation but costs sample size (e.g. 84 days → 12 points)
  and day-of-week structure — pre-commit the granularity for a stated reason, or include weekly as
  one *pre-declared* spec inside the SCA grid.
...
const DATA = {
  specs: {
    ...
    best: {
      ...
      effect: 227000,       // cumulative $
      daily: 56800,         // daily avg $
...
**Always cross-check masking against the empirical-null backtest RANK and the spec grid**
(and, optionally, a date-shuffled test on 50+ random dates). A divergence — BSTS p<0.05 but
date-shuffled p>0.10 — is a flag to *diagnose*, not proof of overconfidence: on masked/calm data
the date-shuffled test can itself test the wrong question. Run the two cheap diagnostics first
(training-length-matched permutation + mask-off rolling placebo — see "Diagnose, don't demote"),
then report the spread honestly: "BSTS p=0.017 (naive), backtest rank 0.95 (empirical-null),
date-shuffled p=0.24 (secondary)." Trust the RANK + spec grid over the date-shuffled p when they
disagree.
...
**What the model actually sees (example, the analyst's config 2024-01-01 → 2026-02-26):**

```
2024-10-31   $6,310   trend=30.3   ← last real day
...
2025-01-06   $7,995   trend=51.9   ← real again
```
...
1. **Spurious covariate→target learning.** The model learns "when Fourier/Trends indicate Nov-Dec,
   ...
   prediction.
2. **Inflated pre-period posterior variance.** The model sees revenue swing from ~$7.6K → $0 → ~$7.6K
...
3. **Permutation test amplification.** Shuffled placebo treatments inherit the distorted fit.
   In the retail case study, the top placebo shuffle produced +$1.6M vs a real effect of +$251K
...
- **Option C (simplest):** Deprecate the mask mode entirely. Use only contiguous pre-period
  windows (e.g., "post-Xmas trimmed": Jan 6 → treatment date). This is the approach the retail engagement
  used for its lead spec after discovering the bug.
...
5. **Mask non-informativeness.** If you mask high-variance periods (Nov-Dec, Black Friday), you
   ...
   in the retail engagement was a concrete violation of this** — the masked dates were silently
...
4. **Report the multi-method ensemble as a range, not a point.** If RDiT says $122K, BSTS
   HMC says $228K, and Prophet says $272K, your headline is "$122K–$272K with a median
   around $222K" — not "$228K per BSTS". The dispersion is itself information.
...
on a real engagement's canonical spec (a UK retailer, post-Issue-#51). The spec is a 24-covariate
...
| Specification grid (224 mask-mode specs) | **208/208 valid specs directional positive**, top 10 per mode cluster $199K–$226K with model p < 0.025 | "Robust across alternatives" |
...
length, the fake-effect SD went from ~$138K at <1-year windows to ~$73K at 1.5-2-year windows,
...
- **Signed median**: `np.median(placebos)` — what `compute_placebo_rank` uses; e.g. $12K on a
  post-fix distribution
- **Absolute median**: `np.median(np.abs(placebos))` — a *different* summary of the same
  distribution; e.g. $77K on the same data
...
| "Median placebo effect: $77K" | "Median placebo effect (signed): $12K. (Do NOT mix with $77K, which is the median of |placebo| — a different quantity.)" |
| "Spec curve analysis validates the effect" | "224-spec robustness grid: 208 valid specs, all directional positive, top 10 per mask mode $199K-$226K" (give the counts, not just the word "validates") |
...
is robust but the methods disagree, report the disagreement *as the dispersion* — "$122K-$272K
with a median around $222K" — rather than picking a favourite. When the spec grid itself
...
**Inconclusive results, equivalence & cross-checks (added v2.4.0):** Altman & Bland (1995),
Lakens (2017), Cumming (2014), Wagner et al. (2002), Young (2019).
